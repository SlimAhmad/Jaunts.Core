...
namespace Jaunts.Core.Api.Tests.Unit.Services.Processings.Role
{
    ...
    {
        [Fact]
        public async Task ShouldThrowValidationExceptionOnUpsertIfRoleIsNullAndLogItAsync()
        {
            // given
            ApplicationRole nullRole = null;

            var nullRoleProcessingException =
                new NullRoleProcessingException(
            ...
            var expectedRoleProcessingValidationException =
                new RoleProcessingValidationException(
                    ...
                    innerException: nullRoleProcessingException);

            // when
            ValueTask<ApplicationRole> upsertRoleTask =
                this.roleProcessingService.UpsertRoleAsync(
                    nullRole);
        ...
        [Fact]
        public async Task ShouldThrowValidationExceptionOnUpsertIfRoleIdIsInvalidAndLogItAsync()
        {
            // given
            var invalidRole = CreateRandomRole();
            invalidRole.Id = Guid.Empty;
            ...
            // when
            ValueTask<ApplicationRole> upsertRoleTask =
                this.roleProcessingService.UpsertRoleAsync(invalidRole);
